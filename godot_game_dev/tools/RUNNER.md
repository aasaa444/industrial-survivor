...
The script fixes the project root, Godot 4.7.1 console executable, exact `res://addons/gdUnit4/bin/GdUnitCmdTool.gd` casing, explicit test files, timeout (120 s), report directory, and exit-code checks. `-DryRun` verifies executable/version/project/runner and prints the exact command without running tests. `-SelfTest` verifies the wrapper path checks without running tests. A real run requires exit 0 and the authoritative `Overall Summary: 82 test cases` / `Executed test cases: (82/82)` markers. Reports are written outside the project under `user://toolchain-suite` (Godot app data). UI/input interaction is not proven by this headless runner; use native runtime QA for that.
